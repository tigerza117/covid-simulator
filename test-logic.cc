...
class member
{
  ...
  public:
        ...
        if (socket->Recv())
        {
            ...
            if (random > memberData[socket->GetNode()->GetId()].get_covid_rate)
            {
                ...
                if (random2 > memberData[socket->GetNode()->GetId()].infected_rate)
                {
                    ...
                    Ptr<MobilityModel> position_infected =
                        node.Get(random_inf)->GetObject<MobilityModel>();
                    ...
                    double distance = sqrt(abs((pow((position_inf_x - position_x), 2.0) +
                                                pow((position_inf_y - position_y), 2.0))));
                    if (distance < 20.0)
                    {
    ...
    void setSentPacket(int node_id)
    {
    ...
    float halfcircle(float num, float y)
    {
        return (137 - (sqrt(pow(5.0, 2.0) - pow(num - y, 2.0))));
    }

    void MovetoRack(NodeContainer c, uint32_t node_id, float x, float y)
    {
        ...
        else if (node_id % 9 == 0)
        {
            ...
            // Move (wayMobility, 0.0, 2.9, 0, 0, x, y);
            // Move (wayMobility, 3.0, 5.9, x, y, before, 2);
            Move(wayMobility, 0.0, 1.9, 0, 0, x, y);
            Move(wayMobility, 6, 9.9, x, y, 137, y);
    ...
    void MoveBack(NodeContainer c, uint32_t node_id, float x, float y)
    {
        Ptr<WaypointMobilityModel> wayMobility;
        wayMobility = c.Get(node_id)->GetObject<WaypointMobilityModel>();
        // float before = x - 4;
        ...
        {
            Move(wayMobility, 35.0, 49.9, 137, y, x, y);
    ...
    void Move(Ptr<WaypointMobilityModel> wayMobility,
              double time_str,
              double time_end,
              float x_str,
              float y_str,
